...
# Security: Allowed IP whitelist (Home Assistant VM)
$allowedIP = "192.168.1.69"

# Dynamically resolve host IPv4 address on the local network
$hostIP = (Get-NetIPAddress -AddressFamily IPv4 -ErrorAction SilentlyContinue | 
           Where-Object { $_.IPAddress -like "192.168.1.*" } | 
           Select-Object -ExpandProperty IPAddress -First 1)
if (-not $hostIP) { $hostIP = "192.168.1.98" }
...
Write-Host "Host IP: $hostIP | Authorized VM: $allowedIP" -ForegroundColor Yellow
...
while ($true) {
    ...
    try {
        if ($client.Available -gt 0) {
            ...
            if ($tempEndpoint.Address.ToString() -ne $allowedIP) { continue }
